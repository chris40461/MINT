...
class KRXDataClient:
    ...
    BLD = {
        ...
        # 투자자별 거래 - 전체시장 기간합계 (pykrx 원본)
        "investor_market_period": "dbms/MDC/STAT/standard/MDCSTAT02201",
    ...
    @retry_on_session_expired()
    def get_market_ticker_name(self, ticker_or_date: Optional[str] = None, market: str = "ALL"):
        """
        종목코드-종목명 매핑 (pykrx 호환)

        pykrx 호환:
        - get_market_ticker_name(ticker) → str (단일 종목명)
        - get_market_ticker_name(date, market) → Dict[str, str] (전체 매핑)

        Args:
            ticker_or_date: 종목코드(6자리) 또는 날짜(YYYYMMDD)
            market: 시장 (ALL/KOSPI/KOSDAQ)

        Returns:
            str: 단일 종목명 (ticker가 6자리인 경우)
            Dict[str, str]: {종목코드: 종목명} (그 외)
        """
        df = self._get_ticker_info(market)
        if df.empty:
            return "" if ticker_or_date and len(ticker_or_date) == 6 else {}

        ticker_name_map = dict(zip(df["short_code"], df["codeName"]))

        # pykrx 호환: 6자리 종목코드면 단일 종목명 반환
        if ticker_or_date and len(ticker_or_date) == 6:
            return ticker_name_map.get(ticker_or_date, "")
    ...
    @retry_on_session_expired()
    def _get_market_trading_value_by_investor_market(
        self,
        fromdate: str,
        todate: str,
        market: str,
        ...
        """
        시장 전체 투자자별 거래대금 조회 (pykrx 호환)

        pykrx 원본 로직 적용:
        - BLD: dbms/MDC/STAT/standard/MDCSTAT02201
        - MultiIndex 컬럼: [['거래량', '거래대금'], ['매도', '매수', '순매수']]

        Args:
            fromdate: 시작일 (YYYYMMDD)
            todate: 종료일 (YYYYMMDD)
            market: 시장코드 (KOSPI/KOSDAQ/KONEX/ALL)
            detail: 상세 투자자 구분 여부 (미사용, pykrx 호환성)
            etf: ETF 포함 여부
            etn: ETN 포함 여부
            elw: ELW 포함 여부

        Returns:
            DataFrame: 투자자별 거래대금 (MultiIndex 컬럼)
                인덱스: 투자자구분 (외국인, 기관합계, 개인 등)
                컬럼: [['거래량', '거래대금'], ['매도', '매수', '순매수']]
        """
        # 시장 코드 매핑 (pykrx 원본과 동일)
        market_map = {
            "KOSPI": "STK",
            "KOSDAQ": "KSQ",
            "KONEX": "KNX",
        ...
        mktId = market_map.get(market.upper(), "ALL")

        # ETF/ETN/ELW 파라미터 (pykrx 형식)
        etf_param = "EF" if etf else ""
        etn_param = "EN" if etn else ""
        elw_param = "EW" if elw else ""

        # API 호출 (올바른 BLD endpoint 사용)
        items = self._request(
            self.BLD["investor_market_period"],
            {
                "strtDd": fromdate,
                "endDd": todate,
                "mktId": mktId,
        ...
        if not items:
            # 빈 DataFrame 반환 (pykrx 형식)
            empty_df = DataFrame()
            empty_df.index.name = '투자자구분'
            empty_df.columns = pd.MultiIndex.from_product([['거래량', '거래대금'],
                                                           ['매도', '매수', '순매수']])
            return empty_df
        ...
        # CONV_OBJ_TP_CD 컬럼 제거
        if 'CONV_OBJ_TP_CD' in df.columns:
            df = df.drop('CONV_OBJ_TP_CD', axis=1)
        ...
        # MultiIndex 컬럼 생성
        # 원본 컬럼 순서: SELN_TRDVOL, SHNU_TRDVOL, NETBID_TRDVOL,
        #                SELN_TRDVAL, SHNU_TRDVAL, NETBID_TRDVAL
        # 매핑: SELN=매도, SHNU=매수, NETBID=순매수
        #       TRDVOL=거래량, TRDVAL=거래대금
        df.columns = pd.MultiIndex.from_product([['거래량', '거래대금'],
                                                 ['매도', '매수', '순매수']])

        # 데이터 정제
        df = df.replace(r'[^-\w]', '', regex=True)  # 쉼표 등 특수문자 제거
        df = df.replace('', '0')  # 빈 값을 0으로
    ...
    def get_market_trading_value_by_investor(
        ...
        Args:
            fromdate: 시작일 (YYYYMMDD)
            todate: 종료일 (YYYYMMDD)
            ticker: 종목코드 또는 시장코드 (KOSPI/KOSDAQ/KONEX/ALL)
            detail: True면 12개 투자자, False면 5개 합산

        Returns:
            DataFrame: 투자자별 순매수금액
                - 개별 종목: 날짜별 DataFrame
                - 시장 전체: 단일 행 DataFrame (MultiIndex 컬럼)
        """
        # 시장 코드 체크
        market_codes = {"KOSPI", "KOSDAQ", "KONEX", "ALL"}
        if ticker.upper() in market_codes:
            # 시장 전체 조회
            df = self._get_market_trading_value_by_investor_market(
                fromdate, todate, ticker.upper()
            )
            # ('거래대금', '순매수') 컬럼만 추출하여 단순 DataFrame 반환
            if not df.empty and ('거래대금', '순매수') in df.columns:
                return df[('거래대금', '순매수')].to_frame(name='순매수')
            return DataFrame()
